...
_product = {
    ...
    i.split('/')[-1]: [] for i in fs.ls(f'noaa-goes16')
}
...
def _check_param_inputs(**params):
    ...
    if product.upper().startswith('ABI'):
        ...
        assert domain in _domain, f'domain must be one of {list(_domain.keys())} or an alias {list(_domain.values())}'
    else:
        domain = None
    ...
    if product not in _product:
        for key, aliases in _product.items():
            if product.upper() in aliases:
                product = key
    assert product in _product, f'product must be one of {list(_product .keys())} or an alias {list(_product .values())}'
...
def _goes_file_df(satellite, product, start, end):
    ...
    DATES = pd.date_range(f"{start:%Y-%m-%d %H:00}",
                          f"{end:%Y-%m-%d %H:00}",
                          freq='1H')
    ...
    df[['start', 'end', 'creation']] = df['file'].str.rsplit('_', expand=True, n=3).loc[:, 1:]
...
def _as_xarray_MP(src, download_dir, i=None, n=None, verbose=True):
    ...
    if local_copy.is_file():
        ...
        if verbose: print(f'\r📖☁ Reading ({i:,}/{n:,}) file from AWS to MEMORY [{src}].', end=' ')
    ...
    attr2coord = ['dataset_name', 'date_created', 'time_coverage_start', 'time_coverage_end']
    for i in attr2coord:
        if i in ds.attrs:
...
def _as_xarray(df, **params):
    ...
    if verbose: print(f"\r{'':1000}\r📚 Finished reading [{n}] files into xarray.Dataset.")
    return ds
###############################################################################
###############################################################################
...
def goes_timerange(start=None, end=None, recent=None, *,
    ...
    if check1:
        assert hasattr(start, 'second') and hasattr(end, 'second'), '`start` and `end` must be a datetime object'
    elif check2:
